...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:80/api/health' -UseBasicParsing -TimeoutSec 5; if($response.StatusCode -eq 200) { exit 0 } else { exit 1 } } catch { exit 1 }"
if %errorlevel% equ 0 (
